...
def _fmt_rfc2822(dt: Optional[datetime]) -> str:
    ...
    if dt.tzinfo is None:
        dt = dt.replace(tzinfo=timezone.utc)
    target_tz = timezone(timedelta(hours=1))
    return format_datetime(dt.astimezone(target_tz))
...
def build_index_html(
    ...
    cineplexx_updated: Optional[datetime] = None,
    telegram_updated: Optional[datetime] = None,
    ...
    def section(title: str, items: list[FeedLink], hint: str = "", extra_html: str = "") -> str:
        if not items:
            return f"""
    ...
    cineplexx_updated_text = _fmt_dt(cineplexx_updated)
    telegram_updated_text = _fmt_dt(telegram_updated)
  ...
  <div class="wrap">
    <header>
      <div class="brand">
        ...
        <div class="meta">Last updated: <code>{escape(updated_text)}</code></div>
      </div>
    ...
    <main id="content">
      {section("Cineplexx", cineplexx, "Cinema feeds generated by Playwright scraper.", f'<p class="muted">Last updated: <code>{escape(cineplexx_updated_text)}</code></p><p class="muted"><a class="link" href="state_location_0.json">state_location_0.json</a></p>')}
      {section("Telegram", telegram, "Public channel feeds parsed from t.me/s/<channel>.", f'<p class="muted">Last updated: <code>{escape(telegram_updated_text)}</code></p>')}
